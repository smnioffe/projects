...
select description,procedure_code, count(*) as occurrence,avg(avg_cost) as avg_cost from(
select distinct member_id,description,procedure_code,  DATEDIFF(week,death_date,service_from_date) as [week], avg(submitted_total_billed_amount) as avg_cost  from(
select distinct pc.*,admitting_diagnosis_unscrubbed,procedure_code,procedure_code_type, procedure_code_type_unscrubbed,description,[code_category] from (
...
join plan_claim_header pch on
 ...
 group by member_id,description,procedure_code,  DATEDIFF(week,death_date,service_from_date)
 ) a group by description,procedure_code
